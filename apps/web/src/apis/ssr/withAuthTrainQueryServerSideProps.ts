...
export const withAuthQueryServerSideProps = (getServerSidePropsFunc?: GetServerSideProps) => {
  return async (context: GetServerSidePropsContextWithAuthClient) => {
    try {
      ...
      if (!getServerSidePropsFunc) {
        ...
        const dehydratedState = JSON.parse(JSON.stringify(dehydrate(context.queryClient)))
