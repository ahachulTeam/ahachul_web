...
const PageTemplate = ({
  ...
  return (
    <React.Fragment>
      {!isPrivatePage && (
        <SEO {...(metaData ? metaData : DEFAULT_SEO_CONFIG)} />
      )}
      {children}
    </React.Fragment>
  );
...
PageTemplate.ContentsSection = function ContentsSection({
  children,
}: PropsWithChildren<PageTemplateProps>) {
...
PageTemplate.ModalOrFloatingContents = function ModalOrFloatingContents({
  ...
  return (
    <ModalOrFloatingContentsStyled>{children}</ModalOrFloatingContentsStyled>
  );
};
