...
const SocialLogin = () => {
  const clickLogin = async (loginType: SocialSignInType) => {
    if (loginType === SocialSignInType.APPLE) {
      ...
      const url = `https://appleid.apple.com/auth/authorize?client_id=${APPLE_CLIENT_ID}&redirect_uri=${APPLE_REDIRECT_URI}&response_type=code&scope=openid&response_mode=form_post`;
