...
import { ScrollDirection, useScrollDirection } from 'shared/lib/hooks/useScrollDirection';
...
export const Navbar: React.FC<NavbarProps> = React.memo(({ elementRef, handleScrollToTop }) => {
  const { scrollDir } = useScrollDirection(elementRef);

  const isVisible = useMemo(() => scrollDir === ScrollDirection.up, [scrollDir]);
