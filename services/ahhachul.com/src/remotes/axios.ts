...
import * as O from "~/utils/fp/option";
import { TokenService } from "~/utils/authentication/tokenService";
  ...
  instance.interceptors.request.use(
    (config) => {
      ...
      if (TokenService.isServer() && tokenService.context?.req?.cookies) {
        const optionCookie = O.fromUndefined(
          tokenService.context.req.cookies[tokenService.cookieKey],
        );

        accessToken = O.mapOrElse(
          optionCookie,
        ...
        requestConfig.headers.Cookie = O.mapOrElse(
          optionCookie,
          (cookie) =>
            `${tokenService.cookieKey}={${encodeURIComponent(cookie.slice(1, -1))}}`,
          "",
        );
    ...
    async (error) => {
      if (isAxiosError(error)) {
        if (error.response && error.response.data) {
          const { status, code } = error.response.data as APIErrorResponse;
