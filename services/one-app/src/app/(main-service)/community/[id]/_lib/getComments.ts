...
type CommunityCommentsQueryKey = ReturnType<typeof communityQueryKeys.comments>;
...
> = async ({ queryKey }) => {
  ...
  const endpoint = `${API_BASE_URL}${API_PATHS.community.comments(id)}?sort=${API_SORT.createdAtAsc}`;
