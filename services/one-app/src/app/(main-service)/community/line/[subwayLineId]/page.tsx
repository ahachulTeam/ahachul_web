...
function buildLineScopedHref(subwayLineId: string, query: SearchQuery, locale: SupportedLocale) {
  const scopedSearch = new URLSearchParams();
  scopedSearch.set('subwayLineId', subwayLineId);
  ...
  if (query.stationId) {
    scopedSearch.set('stationId', query.stationId);
  }
  if (query.lineName) {
    scopedSearch.set('lineName', query.lineName);
  }

  return `${localizePathname(`/community/line/${subwayLineId}`, locale)}?${scopedSearch.toString()}`;
}
...
export async function generateMetadata({ params, searchParams }: Props) {
  ...
  return generateCommunityMetadata(
    Promise.resolve({
      ...query,
      subwayLineId: resolvedParams.subwayLineId,
    }),
  );
}

export default async function CommunityLinePage({ params, searchParams }: Props) {
  const resolvedParams = await params;
  const query = await searchParams;
  const locale = await getServerLocale();
  const messages = getLocaleMessages(locale);
  const scopedHref = buildLineScopedHref(resolvedParams.subwayLineId, query, locale);

  if (query.subwayLineId !== resolvedParams.subwayLineId) {
    redirect(scopedHref);
  }
  ...
  const lineLabel = query.lineName ?? `${resolvedParams.subwayLineId}호선`;
      ...
      <HydrationBoundary state={dehydratedState}>
        <BreadcrumbNav
          items={[
            ...
            { name: `${lineLabel} 커뮤니티`, href: scopedHref },
          ]}
        />
        <SearchForm />
        <Filters />
