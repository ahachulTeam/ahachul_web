...
const COMMENT_IMAGE_KEY_PREFIX = 'comments';
...
function buildCommentImageS3Key(file: File) {
  ...
  if (!extension) {
    return `${COMMENT_IMAGE_KEY_PREFIX}/${randomKey}`;
  }

  return `${COMMENT_IMAGE_KEY_PREFIX}/${randomKey}.${extension}`;
}
...
async function uploadSingleCommentImage(file: File) {
...
export async function uploadCommentImageFiles(files: File[]) {
  const uploadedUrls: string[] = [];

  for (const file of files) {
    const uploadedUrl = await uploadSingleCommentImage(file);
