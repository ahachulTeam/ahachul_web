...
const FilterSection = () => {
  ...
  const handleSubwayLine = (subwayLine: Nullable<string>) => {
    if (!subwayLine) router.push(`${PATH.community}${categoryType ? `?categoryType=${categoryType}` : ''}`);
    else router.push(`${PATH.community}/${subwayLine}${categoryType ? `?categoryType=${categoryType}` : ''}`);
  };
