...
16. FE governance artifacts are append-only and auditable: rulebook edits must be recorded in `FE_RULEBOOK_CHANGELOG.md`, and FE meeting logs must be timestamped files under `docs/fe-meetings`.
...
36. SEO discovery endpoints are contractized and segmented: internal linking must remain crawlable via anchor-based navigation, structured data is emitted from shared builders (`WebSite`/`Organization`/`SiteNavigationElement`/`BreadcrumbList`), sitemap is served as an index plus domain segments, and RSS routes must fail-safe with valid XML output even if upstream API fetch fails during build/runtime.
37. i18n scope is reset to static-only for Next.js: dynamic translation and DeepL usage are out-of-scope, and locale delivery is limited to pre-authored message resources (`ko/en/th/cn`) with shared locale selector UX on home footer and my page.
